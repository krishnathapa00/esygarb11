...
const SubCategories = () => {
  ...
  const getCategoryInfo = (slug: string) => {
    const categoryMap: { [key: string]: { name: string; filter: string } } = {
      'fresh-vegetables': { name: 'Fresh Vegetables', filter: 'Fruits & Vegetables' },
      'organic-fruits': { name: 'Organic Fruits', filter: 'Fruits & Vegetables' },
      'dairy-products': { name: 'Dairy & Eggs', filter: 'Dairy & Eggs' },
      'snacks-beverages': { name: 'Snacks & Beverages', filter: 'Snacks & Beverages' },
  ...
  const filteredProducts = products.filter(p => p.category === categoryInfo.filter);
