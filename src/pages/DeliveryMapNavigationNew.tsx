...
const DeliveryMapNavigationNew = () => {
    ...
    const initMap = () => {
      ...
      try {
        const mapInstance = new window.google.maps.Map(mapContainer.current, {
          zoom: 14,
          center: { lat: 27.7172, lng: 85.3240 }, // Default Kathmandu center
          mapTypeControl: true,
        });
  ...
  useEffect(() => {
    if (partnerLocation && customerLocation && map.current && directionsService.current) {
      updateRoute();
    }
  ...
  const updateRoute = async () => {
    if (!partnerLocation || !customerLocation || !directionsService.current || !directionsRenderer.current) return;
    
    try {
      const request = {
        origin: new window.google.maps.LatLng(partnerLocation.lat, partnerLocation.lng),
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{order.profiles?.full_name || order.user_profile?.full_name || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Phone:</span>
              <span className="font-medium flex items-center gap-2">
                {order.profiles?.phone_number || order.user_profile?.phone_number || 'N/A'}
                {(order.profiles?.phone_number || order.user_profile?.phone_number) && (
                  <a 
            ...
            <div className="space-y-1">
              ...
              <p className="font-medium">{customerLocationName || order.delivery_address}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className={`font-bold text-lg ${orderTimer.isOverdue ? 'text-red-600' : 'text-primary'}`}>
