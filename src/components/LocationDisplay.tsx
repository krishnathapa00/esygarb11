    ...
    const formatLocationName = async (addr: string) => {
      ...
      if (coordPattern.test(addr.trim())) {
        ...
        try {
          ...
          const response = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${addr}.json?access_token=${process.env.VITE_MAPBOX_TOKEN || 'pk.eyJ1IjoidGVzdHVzZXIxMjM0IiwiYSI6ImNsc3l5bjg1YzBibHAyanFtcjV5d3Q4ZnkifQ.0aDn7U_8_lW7rJpe6XnOZA'}`);
          if (response.ok) {
            ...
            if (data.features && data.features.length > 0) {
              ...
              if (parts.length > 0) {
                ...
                setLocationName(placeName.length > 25 ? placeName.substring(0, 22) + '...' : placeName);
              }
            } else {
        ...
        // Already a readable address, just shorten if needed
        const shortAddr = addr.length > 25 ? addr.split(',')[0].trim() + '...' : addr;
